...
const plot = subpopKdePlot({
  ...
  diffData: kdeDiffOrigen,
  bagplotData: bagplotOrigen,
  mode,
  colorMap,
  labelMap,
  displayMode,
...
display(html`<div style="display:flex; gap:20px; align-items:center; margin: 4px 0 12px 0; flex-wrap:wrap;">
  ...
  ${displayMode === "diff" ? html`<span style="display:flex; align-items:center; gap:8px;">
    ...
    <svg width="90" height="14"><defs><linearGradient id="diffgrad-o" x1="0" x2="1"><stop offset="0%" stop-color="#b2182b"/><stop offset="50%" stop-color="#f7f7f7"/><stop offset="100%" stop-color="#2166ac"/></linearGradient></defs><rect x="0" y="1" width="90" height="12" fill="url(#diffgrad-o)" rx="2" stroke="#ccc" stroke-width="0.5"/></svg>
...
Grupo **${labelMap["Centro A"]}** arriba · Grupo **${labelMap["Centro B"]}** abajo · Línea = mediana.

```js
display(subpopViolinPlot({
  selectedFdi,
  violinData: violinOrigen,
  colorMap,
  labelMap,
  width: Math.min(width, 980),
}));
